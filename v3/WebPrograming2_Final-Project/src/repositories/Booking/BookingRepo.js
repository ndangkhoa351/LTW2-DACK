...
Booking.getAllWithUserID = async function (userID) {
  const result = await sequelize.query(
    `SELECT s."startTime" "date", f."displayName" "film_name", c."displayName" "cinema_name", t."chairCode" "chair", f."uuid" "id"
     FROM bookings b JOIN tickets t ON b.uuid = t.booking_id 
          JOIN showtimes s ON s.uuid = b.showtime_id 
          JOIN films f ON f.uuid = s.film_id 
          JOIN users u ON u.uuid = b.user_id 
          JOIN cinemas c ON c.uuid = s.cinema_id WHERE u.uuid = '${userID}'`,
